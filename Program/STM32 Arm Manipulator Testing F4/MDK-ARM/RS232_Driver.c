...
/*SEND MOVE COMMAND*/
void Send_move(Data_Get_t* get, Ctrl_Mode_t control_mode, Move_Var_t var_type, Move_Mode_t move_mode, double value){
	uint8_t move_pos[] = {HEADER1, HEADER2, HEADER3, MOVE_POSITION_CMD, control_mode, move_mode, var_type};
	if(move_mode == CONTINUOUS){
		for(int i=0; i<8; i++) move_pos[i+7] = 0x00;
	}
	
	else if(move_mode == DISTANCE || move_mode == STEP){
		uint8_t new_value[8];
...
void Get_command(Data_Get_t* get){
	for(int i=0; i<BUFF_SIZE; i++){
		if(get->data_buff[i] == HEADER1 && get->data_buff[i+1] == HEADER2 && get->data_buff[i+2] == HEADER3){
			...
			if(get->data_buff[i+3] == AUTO_HOME_CMD){
				get->type = AUTO_HOME;
			}
			
			//CHECK MAPPING MODE COMMAND
			else if(get->data_buff[i+3] == MAPPING_MODE_CMD){
				get->type = MAPPING;
			}
			...
			//CHECK MOVE COMMAND
				...
				get->move_variable = (Move_Var_t) get->data_buff[i+5];
				get->move_mode = (Move_Mode_t) get->data_buff[i+6];
			
				uint8_t move_variable[8];
				
				for(int j=0; j<8; j++) move_variable[j] = get->data_buff[i+7+j];
			...
			//CHECK SEND REQ COMMAND
				...
				for(int j=0; j<8; j++){
					converted_Xpos[j] 	= get->data_buff[j+4]; 	// 4th byte	 - 11th byte
					converted_Ypos[j] 	= get->data_buff[j+12]; // 12th byte - 19th byte
					converted_Zpos[j] 	= get->data_buff[j+20]; // 20th byte - 27th byte
					converted_Joint1[j] = get->data_buff[j+28]; // 28th byte - 35th byte
					converted_Joint2[j] = get->data_buff[j+36]; // 36th byte - 43rd byte
					converted_Joint3[j] = get->data_buff[j+44]; // 44th byte - 51st byte 
					converted_Joint4[j] = get->data_buff[j+52]; // 52nd byte - 59th byte
					converted_Joint5[j] = get->data_buff[j+60]; // 60th byte - 67th byte
					converted_Joint6[j] = get->data_buff[j+68]; // 68th byte - 76nd byte
				}
				...
				get->welding_point = get->data_buff[i+77];
				get->pattern_type = (Welding_Pattern_t) get->data_buff[i+78];
				get->welding_speed = get->data_buff[i+79];	
			}
			...
			else if(get->data_buff[i+3] == STANDBY_CMD){
				get->type = STANDBY;
			}
			
			// CHECK NO COMMAND RECEIVED
			else if(get->data_buff[i+3] == 0x00){
				get->type = NONE;
			}
		}
	}
