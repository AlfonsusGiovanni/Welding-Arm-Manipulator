...
void Send_mapping(Data_Get_t* get, uint8_t point_num, Data_type_t point_type, Welding_Pattern_t pattern_type, Speed_t welding_speed, Mapping_State_t map_state){
	uint8_t mapping_data[] = {HEADER1, HEADER2, HEADER3, MAPPING_CMD, point_num, point_type, pattern_type, welding_speed, map_state};	
...
void Send_running(Data_Get_t* get, Run_State_t state, Run_Mode_t mode, uint16_t point_num){
	uint8_t run[] = {HEADER1, HEADER2, HEADER3, RUNNING_CMD, state, mode, point_num};	
	HAL_UART_Transmit_IT(get->huart, run, sizeof(run));
}

/*SEND DATA REQUEST COMMAND*/
void Req_data(Data_Get_t* get, Req_t data){
	uint8_t req[] = {HEADER1, HEADER2, HEADER3, REQ_DATA_CMD, data};	
...
void Send_motor_state(Data_Get_t* get, Motor_State_t state){
	uint8_t motor_state[] = {HEADER1, HEADER2, HEADER3, MOTOR_STATE_CMD, state};		
...
void Send_welder_state(Data_Get_t* get, Welder_State_t state){
	uint8_t welder_state[] = {HEADER1, HEADER2, HEADER3, WELDER_STATE_CMD, state};	
...
void Send_feedback(Data_Get_t* get, Feedback_t fdbck){
	uint8_t feed_buff[] = {HEADER1, HEADER2, HEADER3, FEEDBACK_CMD, fdbck};		
	HAL_UART_Transmit_IT(get->huart, feed_buff, sizeof(feed_buff));
}

/*SEND STANDBY COMMAND*/
void Send_state_reset(Data_Get_t* get){
	uint8_t standby_buff[] = {HEADER1, HEADER2, HEADER3, RESET_CMD};		
