...
Welding_Data_t current_welding_data;
Welding_Data_t next_welding_data;
// ------------------------------------
...
float
max_delta_angle,
input_linear_pts[2][6],
input_spline_pts[3][3];
...
void read_welding_pattern(uint16_t point, Welding_Data_t *select_point);
void save_welding_speed(uint16_t point, uint8_t select_speed);
void read_welding_speed(uint16_t point, Welding_Data_t *select_point);
void save_welding_axis_offset(uint16_t point, uint8_t select_axis);
void read_welding_axis_offset(uint16_t point, Welding_Data_t *select_point);
void delete_welding_data(uint16_t point, uint8_t point_type);
void get_welding_data(uint16_t point, Welding_Point_t point_type);
...
float get_ang_array(uint8_t select_joint, uint8_t  select_point);
//-----------------------------------------------------------------------------------------------------
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim){
	for(int i=0; i<JOINT_NUM; i++){
		...
		#ifdef USE_TARGET_PRELOAD
		// Preload Next Target Step 
		if((step_counter[i] >= target_step[i] - 10) && robot_run){
			preload_target_step[i] = calc_stepper_step(i, get_ang_array(i, independent_point[i]));
			if(preload_target_step[i] != 0) preload_done[i] = true;
			else preload_done[i] = false;
		}
		
		// Update Target Step
		if(step_counter[i] > target_step[i] && (step_cont_run_w[i] || robot_run)){
			if(robot_run){
				if(preload_done[i]){
					step_counter[i] = 0;
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim){
	for(int i=0; i<JOINT_NUM; i++){
		...
		if(period_time[i] != 0){
			...
			if(raw_joint_angle[i] >= -0.0001f && raw_joint_angle[i] <= 0.0001f) raw_joint_angle[i] = 0.00f;
...
//	get_welding_data(1, BOTH_POINT_DATA);

	validate_welding_point = true;
	
	if(validate_welding_point){
	...
	kinematics.j5_enc_angle = -90.0f;
	run_forward_kinematic(&kinematics, home_ang);
	#endif
...
//		check_singularity(&kinematics);
		...
		#ifdef TEST_MOTION
//		linear_move(welding_data.array_ang_start, welding_data.array_ang_end, LOW);
		float dummy_start[6] = {750.0f, 100.0f, 10.0f, 180.0f, 90.0f, 180.0f};
		float dummy_end[6] = {750.0f, -100.0f, 10.0f, 180.0f, 90.0f, 180.0f};
		spline_move(dummy_start, dummy_end, OFFSET_ON_Z_AXIS, MIDDLE_OFFSET, LOW);
		#endif
				...
				for(int i=0; i<AXIS_NUM-3; i++){	
					if(command.move_variable == robot_axis[i]){			
						...
						if(command.move_mode == CONTINUOUS){
							...
							if(command.move_sign == UNSIGNED_VAR) rx_move_position[i] = 10.0f;
							else if(command.move_sign == SIGNED_VAR) rx_move_position[i] = -10.0f;
						}
...
void read_welding_pattern(uint16_t point, Welding_Data_t *select_point){
	...
	select_point->welding_pattern = (Welding_Pattern_t)read_pattern[0];
	HAL_Delay(5);
}
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
void read_welding_speed(uint16_t point, Welding_Data_t *select_point){
	...
	select_point->welding_speed = (Speed_t)read_speed[0];
	HAL_Delay(5);
}
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
void read_welding_axis_offset(uint16_t point, Welding_Data_t *select_point){
	...
	select_point->welding_axis_offset = (Axis_Offset_t)read_axis[0];
	HAL_Delay(5);
}
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
/* --- GET WELDING DATA FUNCTION --- */
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void get_welding_data(uint16_t point, Welding_Point_t point_type){
	if(point_type == CURRENT_POINT_DATA){
		current_welding_data.welding_point = point;
		read_welding_point(point, START_POINT, current_welding_data.array_pos_start, current_welding_data.array_ang_start);
		read_welding_point(point, END_POINT, current_welding_data.array_pos_end, current_welding_data.array_ang_end);
		read_welding_pattern(point, &current_welding_data);
		read_welding_speed(point, &current_welding_data);
		read_welding_axis_offset(point, &current_welding_data);
	}
	
	else if(point_type == NEXT_POINT_DATA){
		next_welding_data.welding_point = point + 1;
		read_welding_point(point + 1, START_POINT, next_welding_data.array_pos_start, next_welding_data.array_ang_start);
		read_welding_point(point + 1, END_POINT, next_welding_data.array_pos_end, next_welding_data.array_ang_end);
		read_welding_pattern(point + 1, &next_welding_data);
		read_welding_speed(point + 1, &next_welding_data);
		read_welding_axis_offset(point + 1, &next_welding_data);
	}
	
	else if(point_type == BOTH_POINT_DATA){
		current_welding_data.welding_point = point;
		read_welding_point(point, START_POINT, current_welding_data.array_pos_start, current_welding_data.array_ang_start);
		read_welding_point(point, END_POINT, current_welding_data.array_pos_end, current_welding_data.array_ang_end);
		read_welding_pattern(point, &current_welding_data);
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Welding_Data_Status_t weldingpoint_validate(uint16_t point){
	// Get Welding Point Data
	get_welding_data(point, CURRENT_POINT_DATA);
	
	// Data Validation
	if(chcksum_sp_validation != 0 && chcksum_ep_validation != 0){
		if(chcksum_sp == chcksum_sp_validation && chcksum_ep == chcksum_ep_validation){
			if(current_welding_data.welding_pattern > 0x00 && current_welding_data.welding_speed > 0x00){
				return WELDING_DATA_VALID;
			}
		}
		else if(chcksum_sp != chcksum_sp_validation || chcksum_ep != chcksum_ep_validation){
			if(current_welding_data.welding_pattern == 0x00 || current_welding_data.welding_speed == 0x00){
...
/* --- INTERPOLATION ANGLE ACCESS --- */
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
float get_ang_array(uint8_t select_joint, uint8_t  select_point){
	switch(select_joint){
		case 0: return int_j1_out[select_point];
		case 1: return int_j2_out[select_point];
		case 2: return int_j3_out[select_point];
		case 3: return int_j4_out[select_point];
		case 4: return int_j5_out[select_point];
		case 5: return int_j6_out[select_point];
	}
	return 0.0f;
}
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
bool spline_move(float *start_pos, float *end_pos, Axis_Offset_t axis_select, float offset, Speed_t speed){
	...
	if(calculate_parameter){
		...
		calc_linear_points(input_linear_pts, steps, ROT_AXES);
		...
		for(int i=0; i<3; i++){
			input_spline_pts[0][i] = start_pos[i];
			input_spline_pts[1][i] = middle_point[i];
			input_spline_pts[2][i] = end_pos[i];
		}
...
void welding_preview(void){
	...
	if(preview_run){
		...
		// From start point move to end point
			...
			if(current_welding_data.welding_pattern == DOT){
				// Do Nothing
			}
		...
		// Move stop
			...
			if(linear_move(current_welding_data.array_ang_end, home_ang, HIGH) == true){
				preview_run = false;
				get_preview_cmd = false;
				robot_run = false;
