...
void Send_preview(Data_Get_t* get, uint16_t point_num, Run_State_t state){
	uint8_t preview[BUFF_SIZE] = {HEADER1, HEADER2, HEADER3, RUNNING_CMD, point_num, state};	
...
void Get_command(Data_Get_t* get){
	...
	for(int i=0; i<BUFF_SIZE; i++){
		if(rx_buff[i] == HEADER1 && rx_buff[i+1] == HEADER2 && rx_buff[i+2] == HEADER3 && get->buff_status == BUFF_ALIGNED){
			...
			//CHECK PREVIEW MODE COMMAND
			else if(rx_buff[i+3] == PREVIEW_MODE_CMD){
				get->type = PREVIEW;
				
				get->preview_point_num = rx_buff[i+4];
				get->preview_state = (Preview_State_t) rx_buff[i+5];
			}
