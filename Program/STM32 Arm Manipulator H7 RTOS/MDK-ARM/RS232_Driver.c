...
void Get_command(Data_Get_t* get){
	...
	for(int i=0; i<BUFF_SIZE; i++){
		if(get->data_buff[i] == HEADER1 && get->data_buff[i+1] == HEADER2 && get->data_buff[i+2] == HEADER3 && get->buff_status == BUFF_ALIGNED){
			//CHECK AUTO HOME COMMAND 
			if(get->data_buff[i+3] == CALIBRATION_CMD){
				get->type = CALIBRATION;
				get->calibration_mode = (Cal_Mode_t) get->data_buff[i+4];
			}
			...
			//CHECK MAPPING MODE COMMAND
				...
				get->welding_point_num = get->data_buff[i+4];
				get->welding_point_type = get->data_buff[i+5];
				get->pattern_type = (Welding_Pattern_t) get->data_buff[i+6];
				get->running_speed = (Speed_t) get->data_buff[i+7];
				get->mapping_state = (Mapping_State_t) get->data_buff[i+8];
			}
			...
			//CHECK START RUNNING COMMAND
				...
				get->preview_point_num = get->data_buff[i+6];
			}
