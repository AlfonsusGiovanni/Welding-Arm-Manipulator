...
void Req_angle(void){
	uint8_t req[BUFF_SIZE] = {HEADER1, HEADER2, HEADER3, REQ_ANG_CMD};
	uint16_t chcksum = checksum_generator(req, sizeof(req));
...
/*SEND REQUESTED POSITION COMMAND*/
void Send_requested_pos(double* value){
	...
	uint8_t send_req[BUFF_SIZE] = {HEADER1, HEADER2, HEADER3, SEND_REQ_CMD, SEND_POSITION};
...
void Send_requested_angle(double* value){
	...
	uint8_t send_req[BUFF_SIZE] = {HEADER1, HEADER2, HEADER3, SEND_REQ_CMD, SEND_ANGLE};
...
void Get_command(Data_Get_t* get){
	for(int i=0; i<sizeof(get_buff); i++){
		if(get_buff[i] == HEADER1 && get_buff[i+1] == HEADER2 && get_buff[i+2] == HEADER3){
			...
			else if(get_buff[i+3] == REQ_POS_CMD){
				get->type = REQ_POS;
			}
			
			//CHECK REQ ANGLE COMMAND
			else if(get_buff[i+3] == REQ_ANG_CMD){
				get->type = REQ_ANG;
			}
