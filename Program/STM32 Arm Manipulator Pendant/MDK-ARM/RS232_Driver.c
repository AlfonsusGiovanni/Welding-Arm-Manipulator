...
void Send_requested_data(Data_Get_t* get, float* cartesian_value, float* joint_value, uint8_t welding_point, Welding_Pattern_t pattern_type, uint8_t welding_speed){	
	...
	//	memcpy(&get->Cartesian_send[0], new_valueX, sizeof(float));
	//	memcpy(&get->Cartesian_send[1], new_valueY, sizeof(float));
	//	memcpy(&get->Cartesian_send[2], new_valueZ, sizeof(float));
	//	
	//	memcpy(&get->Joint_send[0], new_valueA1, sizeof(float));
	//	memcpy(&get->Joint_send[1], new_valueA2, sizeof(float));
	//	memcpy(&get->Joint_send[2], new_valueA3, sizeof(float));
	//	memcpy(&get->Joint_send[3], new_valueA4, sizeof(float));
	//	memcpy(&get->Joint_send[4], new_valueA5, sizeof(float));
	//	memcpy(&get->Joint_send[5], new_valueA6, sizeof(float));
...
void Get_command(Data_Get_t* get){
	...
	if(rx_buff[0] == HEADER1 && rx_buff[1] == HEADER2 && rx_buff[2] == HEADER3 && get->buff_status == BUFF_ALIGNED){
		...
		else if(rx_buff[3] == MAPPING_MODE_CMD){
			get->type = MAPPING;
		}
		...
		//CHECK MOVE COMMAND
			...
			for(int j=0; j<4; j++) move_variable[j] = rx_buff[8+j];
		...
		//CHECK SEND REQ COMMAND
			...
			for(int j=0; j<4; j++){
				...
				converted_Joint6[j] = rx_buff[j+26];
			}
			...
			get->welding_point = rx_buff[37];
			get->pattern_type = (Welding_Pattern_t) rx_buff[38];
			get->welding_speed = rx_buff[39];	
		}
